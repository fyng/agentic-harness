...
## Agent behavior
- Consider whether a simpler chart form communicates the claim more clearly.
- If the repo has no matplotlib style-setting function, create one from the rules above and make it reusable across figures.
...
- End on a synchronous check: render the figure in the foreground and confirm the file exists. Never start a background or cluster job and wait on it — if the figure needs one, stop and hand the caller the exact command, what a pass and a fail look like, and where the output lands.

Report back the file path(s) you wrote or edited and a one-line summary of the figure produced.
