...
1. **Brainstorm**
   ...
   - Resolve ambiguities and decision forks with the user.
...
2. **Plan**
   ...
   - Turn goals into a concrete implementation. State decisions directly.
   - Track the steps with the available task-list tool.
3. **Orchestrate**
   ...
   - End every subagent on a synchronous check: tests, a lint pass, or a short foreground command. Never leave background work running as the subagent's final action.
   - Wait for its completion report before acting.
4. **Consolidate**
   - Keep the task list current.
   - Take over any task a subagent could not complete.
   - Launch and monitor background work the subagent handed back.

**Background work belongs to the main agent**: the `implementation` agent stops at its last synchronous check and hands background work (SLURM jobs, servers, long runs) back to the caller. The main agent is responsible for launching, monitoring, and interpreting that handoff.

Rules:
- Never repeat a running subagent's work. If its report looks incomplete or stale, follow up with that subagent instead of redoing it.
...
- Do not narrate routine work. Report important findings or changes of direction.
